...
import { Bell, Zap, Image as ImageIcon, AlertTriangle, X, CheckCheck, ExternalLink } from 'lucide-react';
    ...
    return (
        <header className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-dark-300/40 backdrop-blur-sm flex-shrink-0">
            ...
            <div className="flex items-center gap-3">
                ...
                {profile?.role === 'admin' && (
                    <a href="/admin" className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg glass-card border border-purple-500/20 hover:border-purple-500/50 transition-colors">
                        <span className="text-sm font-semibold text-purple-400">Admin Panel</span>
                    </a>
                )}

                {/* Credit indicator */}
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg glass-card border border-brand-500/20">
                    <Zap size={14} className="text-brand-400" />
                    <span className="text-sm font-semibold text-brand-300">{credits}</span>
                            ...
                            {notifications.length > 0 && (
                                <div className="px-4 py-3" style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
                                    <a href="/dashboard/gallery"
                                        ...
                                        View all in Gallery <ExternalLink size={11} />
                                    </a>
                                </div>
                            )}
                        </div>
